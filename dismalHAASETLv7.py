            ...
            for table in cnc_tables:
                src_cursor.execute(f"""
                    SELECT Timestamp, Mode, RunStatus, Program, M30Counter1, ActiveAlarms, EmergencyStop,
                           RapidOverride, SpindleSpeedOverride, FeedrateOverride, SpindleSpeed
                    FROM {table}
                ...
                # --- Convert EmergencyStop to numeric values for alarms ---
                emergency_stop_raw = str(row.get("EmergencyStop", "")).strip().upper()
                if emergency_stop_raw == "TRIGGERED":
                    emergency_stop_val = 1
                elif emergency_stop_raw == "ARMED":
                    emergency_stop_val = 0
                else:
                    emergency_stop_val = None  # fallback if missing/unexpected

                machine_name = table
                timestamp = row["Timestamp"]

                # --- Blanket table insert (cnc_alerts) ---
                insert_sql = """
                    ...
                    ON DUPLICATE KEY UPDATE 
                        ...
                        RapidOverride = VALUES(RapidOverride),
                        SpindleSpeedOverride = VALUES(SpindleSpeedOverride),
                        FeedrateOverride = VALUES(FeedrateOverride),
                ...
                values = (
                    ...
                    row["ActiveAlarms"],
                    emergency_stop_val,
                    row.get("RapidOverride"),
                    row.get("SpindleSpeedOverride"),
                    row.get("FeedrateOverride"),
                    row.get("SpindleSpeed")
                )
                try:
                ...
                create_table_sql_full = f"""
                    CREATE TABLE IF NOT EXISTS {individual_table} (
                        ...
                        RapidOverride FLOAT DEFAULT NULL,
                        SpindleSpeedOverride FLOAT DEFAULT NULL,
                        FeedrateOverride FLOAT DEFAULT NULL,
                        SpindleSpeed FLOAT DEFAULT NULL,
                        PRIMARY KEY (machine_name)
                    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci
                """
                try:
                    tgt_cursor.execute(create_table_sql_full)
                except mysql.connector.Error as err:
